...
public class PostController {
    ...
    public PostResponseDto createPost(@RequestBody PostRequestDto postRequestDto, @AuthenticationPrincipal UserDetailsImpl userDetails) { // @RequestBody BoardrequestDto boardrequestDto : Board에 있는 DB에 저장
        return postService.createPost(postRequestDto, userDetails.getUsername());  // 어떤 사람이 썼는지 사용자의 정보를 가져온다. HttpServletRequest request: 이것은 사용자의 정보부분
    }
    ...
    public ResponseDto delete(@PathVariable Long id, @AuthenticationPrincipal UserDetailsImpl userDetails) { // @RequestBody BoardrequestDto boardrequestDto : 쿠키를 가져오기 위해 사용
        return postService.delete(id, userDetails.getUsername());
    }
